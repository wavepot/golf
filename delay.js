...
export default function Delay(buffer){
  if (!(this instanceof Delay)) return new Delay(buffer);
  this.size = buffer.length //size || 512;
  this.buffer = buffer //new Float32Array(size);
  this.counter = 0;
  this._feedback = 0.5;
  this._delay = 100;
}
